(function () {
  ...
  const downloadDetailedBtn = document.getElementById("downloadDetailed");
...
`;
  ...
  function renderDetailedReport(data) {
    ...
    lastDetailedReport = data;
  }

  function downloadDetailedReport() {
    if (!lastDetailedReport) return;
    const d = lastDetailedReport;
    const lines = [];
    lines.push("RESUME × ROLE FIT — DETAILED ANALYSIS");
    lines.push("======================================");
    lines.push("");
    lines.push("Match: " + (d.match_percentage ?? "—") + "%");
    lines.push("Selection chance: " + (d.selection_chance ?? "—") + "%");
    lines.push("");
    if (d.point_wise_analysis?.length) {
      lines.push("POINT-WISE ANALYSIS");
      lines.push("-------------------");
      d.point_wise_analysis.forEach((p) => lines.push("• " + p));
      lines.push("");
    }
    ...
    if (d.gaps_with_score?.length) {
      lines.push("GAPS (score & description)");
      lines.push("--------------------------");
      d.gaps_with_score.forEach((g) => {
        const skill = g.skill || g;
        const score = g.score || "";
        const desc = g.description || "";
        lines.push("• " + skill + " — " + score + ": " + desc);
      });
      lines.push("");
    }
    if (d.deciding_factors?.length) {
      lines.push("MAJOR DECIDING FACTORS");
      lines.push("----------------------");
      d.deciding_factors.forEach((x) => lines.push("• " + x));
      lines.push("");
    }
    if (d.possible_questions?.length) {
      lines.push("QUESTIONS YOU MIGHT GET ASKED");
      lines.push("---------------------------");
      d.possible_questions.forEach((q) => lines.push("• " + q));
      lines.push("");
    }
    if (d.tips?.length) {
      lines.push("TIPS TO STRENGTHEN YOUR RESUME");
      lines.push("------------------------------");
      d.tips.forEach((t) => lines.push("• " + t));
      lines.push("");
    }
    if (d.skills_to_prepare?.length) {
      lines.push("RELEVANT SKILLS TO PREPARE");
      lines.push("---------------------------");
      d.skills_to_prepare.forEach((st) => {
        lines.push((st.skill || st) + ":");
        (st.topics || []).forEach((tp) => lines.push("  • " + tp));
        lines.push("");
      });
    }
    ...
    a.href = url;
    a.download = "resume-role-fit-detailed-report.txt";
    a.click();
  ...
  if (downloadDetailedBtn) {
    downloadDetailedBtn.addEventListener("click", () => {
      if (!lastDetailedReport) {
        setStatus("Run a detailed analysis first, then download.", true);
        return;
      }
